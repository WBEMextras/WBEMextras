...
echo "#Next 3 lines cannot be used as certificate has a size of 2K and SMH expects 1K certificate"
...
echo "#we need to pull the certificate from port 280 of the SIMServer to get the 1K version of the certificate for SMH"
echo "which wget 1>/dev/null && wget ${SimServer}:280/GetCertificate?certtype=sso -O /var/opt/hpsmh/certs/${SimServer}.pem 2>/dev/null"
echo "if [ ! -f /var/opt/hpsmh/certs/${SimServer}.pem ]; then"
echo "which curl  1>/dev/null && curl --url ${SimServer}:280/GetCertificate?certtype=sso --output /var/opt/hpsmh/certs/${SimServer}.pem"
echo "else"
echo "echo \# We could not download the ${SimServer}.pem certificate."
echo "fi"
